...
from extract_features import get_latent
import os
...
def reduce_dim(x, **kwargs):
    x = get_latent(np.stack(x, -1), **kwargs)[0]
    x = x.transpose(2, 0, 1)
    return x
...
def cluster_hierarchical(
    ...
    if reduce_dimension:
        x_major = reduce_dim(x_major, method='pca', n_components=0.99)
        if x_minor is not None:
            x_minor = reduce_dim(x_minor, method='pca', n_components=0.99)
...
def cluster_mbkmagglo(x, n_clusters):
    ...
    print(int(time() - t0), 'sec')
    lab_cent, __ = sort_labels(lab_cent)
...
def main():
    ...
    if args.n_components is not None:
        x = reduce_dim(x, method='pca', n_components=args.n_components)

    if args.filter_size is not None:
        t0 = time()
        print('Smoothing embeddings...')
        x = smooth(x, args.filter_size)
        print(int(time() - t0), 'sec')

    labels, __ = cluster(x, method=args.method, n_clusters=args.n_clusters)
    # labels, model = cluster_rescale(
    #         embs['cls'],
    #         stride=args.stride,
    #         method=args.method, n_clusters=args.n_clusters)
    # labels, model = cluster_mbkmagglo(
    #         embs['cls'],
    #         n_clusters=args.n_clusters)
    # if args.method == 'agglomerative':
    #     plot_dendrogram(model, outfile=args.prefix+'dendrogram.png')

    # ================== APPLY MASK ==================
    mask_path = args.prefix + "mask-small.png"
    if os.path.exists(mask_path):
        print(f"Applying mask: {mask_path}")
        mask_img = Image.open(mask_path).convert("L")
        mask_arr = np.array(mask_img) > 0   # True inside mask, False outside

        # Ensure mask matches label dimensions
        if mask_arr.shape != labels.shape:
            raise ValueError(
                f"Mask shape {mask_arr.shape} does not match labels shape {labels.shape}"
            )

        labels[~mask_arr] = -1  # set outside-mask regions to -1

    # Remap labels to contiguous values (ignore -1 background)
    valid = labels >= 0
    unique_labels = np.unique(labels[valid])
    remap = {old: new for new, old in enumerate(unique_labels)}

    labels_remapped = np.full_like(labels, fill_value=-1)
    for old, new in remap.items():
        labels_remapped[labels == old] = new


    save_pickle(labels_remapped, args.prefix+'iSCALE_output/clusters-hist/labels.pickle')
    plot_labels(labels_remapped, args.prefix+'iSCALE_output/clusters-hist/labels.png', cmap='tab10')
    plot_label_masks(labels_remapped, args.prefix+'iSCALE_output/clusters-hist/masks/')
